         ...
         <div className='lg:w-[85%] md:w-[70%] h-[100%] lg:ml-[310px] md:ml-[250px] mt-[70px] flex flex-col gap-[30px] overflow-x-hidden py-[50px] ml-[100px]'>
          <div className='w-[400px] h-[50px] text-[28px] md:text-[40px] mb-[20px] text-white'>All Orders List</div>
          {
            orders.map((order, index)=>(
              <div key={index} className='w-[90%] h-[40%] bg-slate-600 rounded-xl flex lg:items-center items-start justify-between flex-col lg:flex-row p-[10px] md:p-[20px] gap-[20px] '>
                ...
                <div className='text-[15px] text-green-100'>
                  ...
                  <p>Date : {new Date(order.date).getDate()}/{new Date(order.date).getMonth() + 1}/{new Date(order.date).getFullYear()}</p>
                  <p className='text-[20px] text-white font-semibold'> ₹ {order.amount}</p>
                </div>

                <select value={order.status} className='px-[5px] py-[10px] outline-0 cursor-pointer bg-slate-500 rounded-lg border-[1px] border-[#96eef3]' onChange={(e)=>statusHandler(e, order._id)}>
